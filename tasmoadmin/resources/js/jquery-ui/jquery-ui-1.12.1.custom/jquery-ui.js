 ...
 */
		 ...
		 */
			...
			_isRootNode: function ( element ) {
				return (
					       /(html|body)/i
				       ).test( element.tagName ) || element === this.document[ 0 ];
			},
		...
		$.ui.plugin.add( "draggable", "scroll", {
			...
			drag : function ( event, ui, i ) {
				...
				if ( scrollParent !== document && scrollParent.tagName !== "HTML" ) {
					if ( !o.axis || o.axis !== "x" ) {
						if ( (
							     i.overflowOffset.top + scrollParent.offsetHeight
						     ) - event.pageY <
						     o.scrollSensitivity ) {
					...
					if ( !o.axis || o.axis !== "y" ) {
						if ( (
							     i.overflowOffset.left + scrollParent.offsetWidth
						     ) - event.pageX <
						     o.scrollSensitivity ) {
					...
					switch ( toleranceMode ) {
						...
						case "intersect":
							return (
								...
								t < y1 + (
									draggable.helperProportions.height / 2
								) && // Bottom Half
								y2 - (
									draggable.helperProportions.height / 2
								) < b
		...
		$.widget( "ui.resizable", $.ui.mouse, {
			...
			_mouseStop: function ( event ) {
				...
				if ( this._helper ) {
					...
					s    = {
						width : (
							that.helper.width() - soffsetw
						),
						height: (
							that.helper.height() - soffseth
						)
					};
					left = (
			...
			_getPaddingPlusBorderDimensions: function ( element ) {
				...
				for ( ; i < 4; i++ ) {
					widths[ i ] = (
						parseFloat( borders[ i ] ) || 0
					);
					widths[ i ] += (
						parseFloat( paddings[ i ] ) || 0
					);
				}
		...
		$.ui.plugin.add( "resizable", "containment", {
			...
			start: function () {
				...
				if ( /document/.test( oc ) || oc === document ) {
					...
					width  = (
						that._hasScroll( ce, "left" ) ? ce.scrollWidth : cw
					);
					height = (
						that._hasScroll( ce ) ? ce.scrollHeight : ch
					);
			...
			_mouseDrag: function ( event ) {
				...
				this.selectees.each( function () {
					...
					if ( options.tolerance === "touch" ) {
						hit = (
							!(
								offset.left > x2 || offset.right < x1 || offset.top > y2 ||
								offset.bottom < y1
							)
						);
					} else if ( options.tolerance === "fit" ) {
						hit = (
							offset.left > x1 && offset.right < x2 && offset.top > y1 &&
							offset.bottom < y2
						);
					}
				...
				switch ( event.keyCode ) {
					...
					default:
						...
						character = event.keyCode >= 96 && event.keyCode <= 105 ?
						            (
							            event.keyCode - 96
						            ).toString() : String.fromCharCode( event.keyCode );
   ...
   allowing multiple different settings on the same page. */
	 ...
	 */
			...
			_newInst: function ( target, inline ) {
				...
				return {
					...
					dpDiv      : (
						!inline ? this.dpDiv : // presentation div
	 ...
	 */
			_destroyDatepicker: function ( target ) {
				...
				if ( nodeName === "input" ) {
					inst.append.remove();
					inst.trigger.remove();
					$target.removeClass( this.markerClassName ).off( "focus", this._showDatepicker ).off(
						"keydown",
						this._doKeyDown
	 ...
	 */
			...
			_shouldFocusInput: function ( inst ) {
				return inst.input && inst.input.is( ":visible" ) && !inst.input.is( ":disabled" ) && !inst.input.is(
					":focus" );
			},
	 ...
	 */
			...
			_ticksTo1970: (
				(
					(
						1970 - 1
					) * 365 + Math.floor( 1970 / 4 ) - Math.floor( 1970 / 100 ) +
					Math.floor( 1970 / 400 )
				) * 24 * 60 * 60 * 10000000
			),
	 ...
	 */
			...
			_setDateFromField: function ( inst, noDefault ) {
				...
				inst.drawYear = inst.selectedYear = date.getFullYear();
				inst.currentDay   = (
					dates ? date.getDate() : 0
				);
				inst.currentMonth = (
					dates ? date.getMonth() : 0
				);
				inst.currentYear  = (
	 ...
	 */
			...
			_generateHTML: function ( inst ) {
				...
				prevText = (
					!navigationAsDateFormat ? prevText : this.formatDate(
						prevText,
				...
				nextText = (
					!navigationAsDateFormat ? nextText : this.formatDate(
						nextText,
						this._daylightSavingAdjust( new Date( drawYear, drawMonth + stepMonths, 1 ) ),
						this._getFormatConfig( inst )
					)
				...
				currentText = (
					!navigationAsDateFormat ? currentText :
				...
				for ( row = 0; row < numMonths[ 0 ]; row++ ) {
					...
					for ( col = 0; col < numMonths[ 1 ]; col++ ) {
						...
						for ( dRow = 0; dRow < numRows; dRow++ ) { // create date picker rows
							calender += "<tr>";
							tbody = (
								!showWeek ? "" : "<td class='ui-datepicker-week-col'>" +
							...
							for ( dow = 0; dow < 7; dow++ ) { // create date picker days
								...
								tbody += "<td class='" +
								         ...
								         (
									         ...
									         ) && daySettings[ 2 ] ? " title='" + daySettings[ 2 ].replace(
										         /'/g,
										         "&#39;"
									         ) + "'" : ""
								         ) + // cell title
				...
				if ( !inst.yearshtml ) {
					...
					if ( secondary || !changeYear ) {
						...
						determineYear = function ( value ) {
							var year = (
								value.match( /c[+\-].*/ ) ? drawYear + parseInt( value.substring( 1 ), 10 ) :
								(
							...
							return (
								isNaN( year ) ? thisYear : year
							);
						};
						year          = determineYear( years[ 0 ] );
						endYear       = Math.max( year, determineYear( years[ 1 ] || "" ) );
						year          = (
							minDate ? Math.max( year, minDate.getFullYear() ) : year
						);
						endYear       = (
							maxDate ? Math.min( endYear, maxDate.getFullYear() ) : endYear
						);
						inst.yearshtml
			...
			_restrictMinMax: function ( inst, date ) {
				...
				return (
					maxDate && newDate > maxDate ? maxDate : newDate
				);
			},
					...
					$.each( items, function ( index, item ) {
						...
						if ( item.optgroup !== currentOptgroup ) {
							...
							that._addClass( li, "ui-selectmenu-optgroup", "ui-menu-divider" +
							                                              (
								                                              item.element.parent( "optgroup" ).prop(
									                                              "disabled" ) ?
								                                              " ui-state-disabled" :
								                                              ""
							                                              ) );
 ...
 */
		...
		var widgetsSlider = $.widget( "ui.slider", $.ui.mouse, {
			...
			_createHandles: function () {
				...
				handleCount = (
					              options.values && options.values.length
				              ) || 1;
		...
		(
			function () {
				...
				$.fx.step.clip = function ( fx ) {
					...
					$( fx.elem ).cssClip( {
						                      top   : fx.pos * (
							                      fx.end.top - fx.start.top
						                      ) + fx.start.top,
						                      right : fx.pos * (
							                      fx.end.right - fx.start.right
						                      ) + fx.start.right,
						                      bottom: fx.pos * (
							                      fx.end.bottom - fx.start.bottom
						                      ) + fx.start.bottom,
						                      left  : fx.pos * (
							                      fx.end.left - fx.start.left
						                      ) + fx.start.left
					                      } );
				};
 ...
 */
		...
		var effectsEffectSize = $.effects.define( "size", function ( options, done ) {
			...
			if ( origin ) {
				...
				from.top  = (
					            original.outerHeight - from.outerHeight
				            ) * baseline.y + pos.top;
				from.left = (
					            original.outerWidth - from.outerWidth
				            ) * baseline.x + pos.left;
				to.top    = (
					            original.outerHeight - to.outerHeight
				            ) * baseline.y + pos.top;
				to.left   = (
					            original.outerWidth - to.outerWidth
				            ) * baseline.x + pos.left;
			}
			element.css( from );
		...
		var effectsEffectScale = $.effects.define( "scale", function ( options, done ) {
			...
			var el         = $( this ),
			    ...
			    percent    = parseInt( options.percent, 10 ) ||
			                 (
				                 parseInt( options.percent, 10 ) === 0 ? 0 : (
					                 mode !== "effect" ? 0 : 100
				                 )
			                 ),
