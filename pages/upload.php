...
if( isset( $_POST[ "upload" ] ) ) {
	try {
		...
		if( !move_uploaded_file(
			...
			throw new RuntimeException(
				__( "UPLOAD_FIRMWARE_MINIMAL_COULD_NOT_SAVE", "DEVICE_UPDATE", [ "FWPath" => $minimal_firmware_path ] )
			);
		}
		
		$msg .= __( "UPLOAD_FIRMWARE_MINIMAL_LABEL", "DEVICE_UPDATE" ) . __(
				"UPLOAD_FIRMWARE_MINIMAL_SUCCESSFULLY", "DEVICE_UPDATE"
			) . "</br>";
...
if( isset( $_POST[ "upload" ] ) ) {
	try {
		...
		if( !isset( $_FILES[ 'minimal_firmware' ][ 'error' ] )
		    || is_array( $_FILES[ 'minimal_firmware' ][ 'error' ] ) ) {
			throw new RuntimeException( __( "UPLOAD_INVALID_FILES", "DEVICE_UPDATE" ) );
		}
		...
		switch( $_FILES[ 'minimal_firmware' ][ 'error' ] ) {
			case UPLOAD_ERR_OK:
				break;
			case UPLOAD_ERR_NO_FILE:
				throw new RuntimeException( __( "UPLOAD_ERR_NO_FILE", "DEVICE_UPDATE" ) );
			case UPLOAD_ERR_INI_SIZE:
			case UPLOAD_ERR_FORM_SIZE:
				throw new RuntimeException( __( "UPLOAD_ERR_FORM_SIZE", "DEVICE_UPDATE" ) );
			default:
				throw new RuntimeException( __( "UPLOAD_UNKNOWN_ERROR", "DEVICE_UPDATE" ) );
		}
