...
$( document ).on( "ready", function () {
	...
	log( "", "", $( "BLOCK_GLOBAL" ), "Update Start", "success" );
	
	$.each( device_ids, step1 );
	
	
	function step1( index, id ) {
		log( id, 1, $( "BLOCK_GLOBAL" ), $.i18n( "BLOCK_GLOBAL_START_STEP_1" ), "info" );
				        ...
				        if ( data && !data.ERROR ) {
					        if ( data.WARNING ) {
						        log(
							        id,
							        step,
							        $.i18n( 'BLOCK_CHECK_CONNECTION' ), $.i18n( 'BLOCK_CHECK_CONNECTION_CHECK_ERROR_MSG' )
							                                            + data.WARNING,
							        "error"
						        );
					        }
